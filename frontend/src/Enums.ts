...
export namespace EnumNotificationType {
    ...
    export const IndividualNotificationType = () => [EnumNotificationType.EMAIL, EnumNotificationType.SMS].map((type) => (type.toString()));

    export const OrganizationNotificationType = () => [EnumNotificationType.API].map((type) => (type.toString()));
}
